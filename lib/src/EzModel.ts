...
export class EzBaseModel<T extends EzBaseModel<T>, TB extends ExtFieldOption = FieldOption> implements IBaseModel<FieldOption> {
  declare $TB: TB
  declare static modelKey: DataKey
}

export const useModelOptions = <T extends EzBaseModel<T>>(instance: BaseModelConstructor<T>) => {
  return EZ_MODEL_POOL[instance.modelKey] as EzModelOptions<T, T['$TB']>
}
...
export const getModelState = <T extends IBaseModel>(constructor: BaseModelConstructor<T>) => {
  return EZ_MODEL_POOL[constructor.modelKey] as EzModelOptions<T, ExtFieldOption>
}
